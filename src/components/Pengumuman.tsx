          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-8">
            {announcements.map((item, idx) => (
              <motion.div 
                ...
                className="group flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 bg-white border border-slate-100 rounded-[20px] shadow-sm hover:shadow-md hover:border-brand-red/30 cursor-pointer transition-all duration-300 relative overflow-hidden"
              >
                ...
                <div className="flex items-start sm:items-center gap-5 w-full">
                  {/* Icon */}
                  <div className="flex-shrink-0 w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center text-brand-red group-hover:scale-110 group-hover:bg-brand-red group-hover:text-white transition-all duration-300">
                    <FileText size={22} strokeWidth={2} />
                  </div>
                  
                  {/* Text Content */}
                  <div className="flex-grow">
                    <span className="block text-[10px] font-bold text-brand-red uppercase tracking-widest mb-1">
                      {item.tag}
                    </span>
                    <h3 className="text-brand-navy font-bold text-base sm:text-lg mb-1 group-hover:text-brand-red transition-colors leading-tight">
                      {item.title}
                    </h3>
                ...
                {/* Arrow Button */}
                <div className="flex-shrink-0 self-end sm:self-auto w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-brand-navy group-hover:text-white transition-all duration-300">
